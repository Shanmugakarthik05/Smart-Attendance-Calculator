...
import {
  ...
  computeHoursPerWeek,
  countDayOccurrences,
  computeSubjectTotalHours,
} from "./components/TimetableManager";
...
export default function App() {
  ...
  // Actual day occurrence counts for the semester
  const dayCounts = countDayOccurrences(startDate, endDate);

  // Compute hours/week per subject from the timetable (for badge display)
  const getComputedHoursPerWeek = (subjectId: string): number => {
    return computeHoursPerWeek(timetable, subjectId, timeSlots);
  };

  const computedHoursMap: Record<string, number> = {};
  subjects.forEach(s => { computedHoursMap[s.id] = getComputedHoursPerWeek(s.id); });

  // Compute exact total scheduled hours based on which days actually occur in the semester
  const getScheduledHoursForSubject = (subjectId: string): number => {
    return computeSubjectTotalHours(timetable, subjectId, dayCounts, timeSlots);
  };
  ...
  // Calculate total hours for a subject (actual days in semester − cancelled)
  const getTotalHoursForSubject = (subjectId: string) => {
    const scheduledHours = getScheduledHoursForSubject(subjectId);
  ...
  const getExportData = () => {
    return subjects.map(subject => {
      const scheduledHours = getScheduledHoursForSubject(subject.id);
      ...
      return {
        ...
        'Hours Per Week': getComputedHoursPerWeek(subject.id),
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-8">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <SubjectManager
          subjects={subjects}
          computedHours={computedHoursMap}
            ...
            <TabsContent value="subjects" className="space-y-6 mt-6">
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {subjects.map((subject) => {
                  ...
                  const cancelledHours = getCancelledHoursForSubject(subject.id);
                  const scheduled = getScheduledHoursForSubject(subject.id);
                  const effectiveSubject = {
                    ...subject,
                    classesPerWeek: getComputedHoursPerWeek(subject.id),
                  };
                  return (
                    <SubjectCard
                      key={subject.id}
