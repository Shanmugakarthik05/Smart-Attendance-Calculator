...
export type Timetable = {
  [day in Day]?: {
    [slot in SlotId]?: string | null; // subjectId or null
  };
};
...
type ActiveCell = { day: Day; slotId: SlotId } | null;

export function TimetableManager({ timetable, subjects, dayCounts, onTimetableChange }: TimetableManagerProps) {
  ...
  const getCell = (day: Day, slotId: SlotId): string | null =>
    timetable[day]?.[slotId] ?? null;

  const setCell = (day: Day, slotId: SlotId, subjectId: string | null) => {
    onTimetableChange({
      ...timetable,
  ...
  const toggleCell = (day: Day, slotId: SlotId) => {
  ...
  const getTotalSemesterHours = (subjectId: string) =>
    computeSubjectTotalHours(timetable, subjectId, dayCounts as Record<Day, number>);

  const isOpen = (day: Day, slotId: SlotId) =>
...
export function computeHoursPerWeek(timetable: Timetable, subjectId: string): number {
  let total = 0;
  for (const day of DAYS) {
    for (const slot of TIME_SLOTS) {
 ...
 */
export function computeHoursPerDay(timetable: Timetable, subjectId: string): Record<Day, number> {
