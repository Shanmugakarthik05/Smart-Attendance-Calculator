...
export function AttendanceDashboard({
  ...
  // Data for pie chart — keys must be unique; values must be > 0
  const pieData = [
    { name: "Attended", value: totalAttended, color: "#22c55e" },
    { name: "Missed", value: totalMissed, color: "#ef4444" },
    { name: "Remaining", value: Math.max(0, totalRemaining), color: "#94a3b8" },
  ].filter((item, i) => item.value > 0);

  // Data for bar chart — use numeric index as XAxis key to guarantee uniqueness
  const barData = subjects.map((subject, i) => {
    const total = getTotalHours(subject.id);
    const attended = subject.attended;
    const missed = subject.missed;
    const remaining = Math.max(0, total - attended - missed);
    const shortName = subject.name.length > 12
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={barData}>
                ...
                <XAxis
                  dataKey="idx"
                  tickFormatter={(i: number) => barData[i]?.shortName ?? ""}
                />
                <YAxis />
                <Tooltip />
                <Legend />
